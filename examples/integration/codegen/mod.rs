...
pub trait BookServiceInterface {
    fn get_book(&self, request: GetBookRequest, ctx: &MyExampleContext) -> Book;
}
...
impl BookServiceCodeGen {
    pub fn register_service<S: BookServiceInterface + Send + Sync + 'static>(
        ...
        service_def.add_definition("GetBook".to_string(), move |request, ctx| {
            let res = service.get_book(GetBookRequest::parse_from_bytes(request).unwrap(), ctx);
