...
/// RpcClient is in charge of creating ports, requesting modules, and sending requests to the `RpcServer`
///
/// To create a `RpcClient`, you should create and pass in a transport in order to communicate with the `RpcServer`
...
impl RpcClient {
    ...
    ///
    /// It will spawn a background task in order to process all responses returned by the `RpcServer`
    ...
    /// Sends a request to the `RpcServer` in order to create a new port for the `RpcClient`
...
/// When the client is dropped, the background task receiving responses from the `RpcServer` should be stopped
...
/// A `RpcClientPort` is in charge of requesting and loading remote modules from the `RpcServer`
///
/// It shares the transport with the `RpcClient`
pub struct RpcClientPort {
    /// Custom name given
    pub port_name: String,
    /// ID for the port given by the `RpcServer`
    port_id: u32,
...
impl RpcClientPort {
    ...
    /// Sends a request to the `RpcServer` requesting a remote module so then the returned module is loaded into memory so that it can be used by the client.
    ///
...
pub struct RpcClientModule {
    ...
    /// ID of the port given by the `RpcServer`
    port_id: u32,
    /// Remote Module's procedures.
    /// - `key` : The name of the procedure.
    /// - `value` : The id of the procedure given by the `RpcServer`
