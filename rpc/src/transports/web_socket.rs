...
///
pub struct WebSocketTransport<Context> {
    ...
    ///
    ready: AtomicBool,
    // TODO: We must change this. Client side doesn't need a context at all.
    ctx: Arc<Context>,
}

// TODO: We must change this. Client side doesn't need a context at all.
impl<Context> WebSocketTransport<Context> {
    /// Crates a new [`WebSocketTransport`] from a websocket connection generated by [`WebSocketServer`] or [`WebSocketClient`]
    pub fn new(ws: WebSocketStream<MaybeTlsStream<TcpStream>>, ctx: Context) -> Self {
...
#[async_trait]
// TODO: We must change this. Client side doesn't need a context at all.
impl<Context: Send + Sync> Transport for WebSocketTransport<Context> {
